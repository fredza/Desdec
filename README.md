...
| View | What you get |
| --- | --- |
| **Overview** | Format, architecture, entry point, SHA-256, entropy, hardening (RELRO, canary, NX, PIE, CFG), detected source language, and every linked library — each with an explanation of what it is for. The entry point leads to the listing, and the section it lands in to the section table. If the file was **packed, protected, virtualised or bundled**, that is said first and with the finding that says it — `section UPX1`, a product's own marker — because the listing of a packed file is its wrapper's, not the program's. With no product named, the shapes that look like one — writable executable code, a tiny import table — are reported as leads, never as a verdict. |
...
| **Compare** | This binary set beside another one, which is the question a reader arrives with when a fix ships and which no other view answers: a recompilation moves the whole image, every address is new, and a byte comparison reports that nothing survived. Functions are paired first — by a name both files carry, by sitting at the same address with the same bytes, by the same bytes anywhere, by the same shape once the numbers instructions carry are set aside, or by being the one unpaired function each side of a pair already made calls — and every row says which of those it was, because a name and a body are facts while a shape and a neighbour are readings. Then what changed: how far apart two bodies are, counted over an alignment that ignores those numbers, so a build at another base address is not reported as a program rewritten; the functions one file has and the other does not; and the strings, the linked libraries and the sections either file holds alone. An ambiguous pairing is refused rather than picked: three identical stubs each side can be matched six ways, and choosing one would be a finding that nothing supports. |
...
`~/.local/share`. On macOS it writes an application bundle into
`~/Applications`, which is what puts Desdec in the Dock, in Spotlight and in
Launchpad. `--no-desktop` installs the binary alone.
...
On Windows it writes a Start menu shortcut with the same icon, asked of the
binary as a `.ico`; `-NoShortcut` installs the binary alone.
...
A third file is yours in the same way: the descriptions you write of the
libraries you meet, at `desdec/libraries.conf`, one `name = explanation` per
line. It overrides the built-in catalogue, and it is edited in Desdec —
Preferences → Behaviour opens it in a box, and the “?” button on an
undescribed library opens the same box with its name already filled in.
Saving reads it back, so the next press says the new sentence.
