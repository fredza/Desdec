...
| --- | --- |
| **Resumen** | Formato, arquitectura, punto de entrada, SHA-256, entropía, endurecimiento (RELRO, canario, NX, PIE, CFG), lenguaje de origen detectado, y cada biblioteca enlazada —con la explicación de para qué sirve. El punto de entrada lleva al listado, y la sección en la que cae a la tabla de secciones. Si el archivo fue **comprimido, protegido, virtualizado o empaquetado**, se dice primero y con el hallazgo que lo dice —`section UPX1`, un marcador del propio producto— porque el listado de un archivo comprimido es el de su envoltura, no el del programa. Sin producto nombrado, las formas que se le parecen —código escribible y ejecutable, tabla de importaciones diminuta— se informan como indicios, nunca como veredicto. |
...
| **Desensamblado** | Listados x86, x86-64 (iced-x86) y AArch64 (Capstone), con edición de los bytes de una instrucción. Con el botón derecho se explica qué designa el operando y qué escribió por última vez en cada registro nombrado. La barra lleva las banderas de condición de la fila seleccionada —las que establece, las que consulta, y aquellas cuyo valor fijan los bytes pasara lo que pasara antes— y una fila sobre la que usted ha escrito se marca al margen. El listado se lee en AT&T o, con una casilla, en NASM: cada fila se descodifica de nuevo a partir de sus propios bytes en lugar de traducirse, de modo que la escritura de Intel es la misma instrucción y no una reescritura del texto. |
...
| **Comparación** | Este binario puesto al lado de otro, que es la pregunta con la que llega el lector cuando sale una corrección y a la que ninguna otra vista responde: una recompilación desplaza toda la imagen, cada dirección es nueva, y una comparación de bytes anuncia que no ha sobrevivido nada. Las funciones se emparejan primero — por un nombre que ambos archivos llevan, por estar en la misma dirección con los mismos bytes, por los mismos bytes estén donde estén, por la misma forma una vez apartados los números que llevan las instrucciones, o por ser la única función sin emparejar que llama cada lado de un par ya establecido — y cada fila dice cuál de esos caminos la produjo, porque un nombre y un cuerpo son hechos mientras que una forma y una vecindad son lecturas. Luego lo que ha cambiado: la distancia entre dos cuerpos, contada sobre una alineación que ignora esos números, para que una compilación en otra dirección base no se anuncie como un programa reescrito; las funciones que un archivo tiene y el otro no; y las cadenas, las bibliotecas enlazadas y las secciones que uno de los dos tiene en solitario. Un emparejamiento ambiguo se rechaza en lugar de elegirse: tres stubs idénticos de cada lado se emparejan de seis maneras, y quedarse con una sería una conclusión que nada sostiene. |
...
Un tercer archivo le pertenece del mismo modo: las descripciones que usted
escribe de las bibliotecas que encuentra, en `desdec/libraries.conf`, una por
línea con la forma `nombre = explicación`. Prevalecen sobre el catálogo
integrado, y el archivo se edita desde Desdec: Preferencias → Comportamiento lo
abre en un cuadro de texto, y el botón «?» de una biblioteca no descrita abre
el mismo cuadro con su nombre ya escrito. Al guardar se relee, de modo que la
siguiente pulsación dice la nueva frase.
