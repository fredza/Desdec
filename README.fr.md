...
| --- | --- |
| **Aperçu** | Format, architecture, point d'entrée, SHA-256, entropie, durcissement (RELRO, canari, NX, PIE, CFG), langage source détecté, et chaque bibliothèque liée — avec l'explication de ce à quoi elle sert. Le point d'entrée mène au listing, et la section où il tombe à la table des sections. Si le fichier a été **compressé, protégé, virtualisé ou empaqueté**, c'est dit en premier et avec le relevé qui le dit — `section UPX1`, un marqueur du produit — parce que le listing d'un fichier compressé est celui de son enveloppe, pas celui du programme. À défaut de produit nommé, les formes qui y ressemblent — code inscriptible et exécutable, table d'imports minuscule — sont rapportées comme des indices, jamais comme un verdict. |
...
| **Fonctions** | Les noms qu'un compilateur a encodés se relisent dans l'orthographe du code source — les deux schémas de Rust, hérité et `v0` — d'un interrupteur au-dessus de la table ; le nom du fichier reste sous le pointeur, et un nom que Desdec ne sait pas relire entièrement est laissé tel quel plutôt qu'à moitié décodé. Les fonctions nommées, leur corps, leurs blocs de base et un graphe de flot de contrôle local. Un clic sur une ligne ouvre le code de cette fonction dans le listing. Un fichier qui n'en nomme aucune a quand même cette vue : ses fonctions sont trouvées à partir de son propre code — le point d'entrée, tout ce que quelque chose appelle, les prologues de compilateur — et chaque ligne dit d'où elle vient, parce qu'une adresse appelée est un fait et un prologue une lecture. À côté de chacune : ce qui l'appelle, ce qu'elle appelle, et les chaînes d'appels les plus courtes qui y mènent depuis un point de départ du fichier — la question « comment on arrive ici ? », à laquelle ni un listing ni une liste de références ne répondent seuls. |
| **Chaînes** | Les chaînes imprimables avec leur décalage et leur encodage, filtrables, et les instructions qui les référencent. |
| **Références** | Ce qui nomme une adresse, pour celle devant laquelle vous êtes. Les appels et les sauts, et ce qu'aucun listing ne montre : l'adresse qu'un AArch64 construit en deux instructions — sans quoi un tel fichier n'a aucune référence à ses données —, les cas d'un `switch` lus dans la table que consulte son `jmp *%rax`, et les vrais appelants d'une fonction importée, derrière le relais que l'éditeur de liens a mis devant elle. Chaque ligne nomme l'endroit où elle se trouve — `main+0x2c` — et dit de quelle force est la réponse : une instruction exacte, une arrivée par une table ou par un relais, ou un mot de données qui n'est peut-être qu'un nombre qui y ressemble. |
| **Désassemblage** | Listings x86, x86-64 (iced-x86) et AArch64 (Capstone), avec édition des octets d'une instruction. Un clic droit explique ce que désigne l'opérande et ce qui a écrit en dernier dans chaque registre nommé. La barre porte les drapeaux de condition de la ligne sélectionnée — ceux qu'elle établit, ceux qu'elle consulte, et ceux dont les octets fixent la valeur quoi qu'il se soit passé avant — et une ligne sur laquelle vous avez écrit est marquée en marge. Le listing se lit en AT&T ou, à une case à cocher près, en NASM : chaque ligne est redécodée depuis ses propres octets plutôt que traduite, de sorte que l'écriture Intel est la même instruction et non une réécriture du texte. |
| **Décompilation** | Du C, reconstruit par Desdec lui-même et sans rien à installer : le cadre de pile devient des variables nommées, la convention d'appel donne les paramètres et les arguments de chaque appel, `cmp` et `jle` deviennent `if (i <= n)`, et les arcs retour deviennent des boucles. Chaque ligne connaît l'instruction dont elle vient, donc un clic l'ouvre — ce qu'aucun moteur externe ne propose, faute de publier une correspondance ligne-adresse. Ce qui n'est pas modélisé reste en commentaire avec son assembleur, et la carte dit quelle part du corps a été comprise. En dessous, la traduction ligne à ligne du binaire entier ; et à la place des deux, la sortie de Rizin/rz-ghidra ou de RetDec si l'un d'eux est installé et choisi. |
...
| **Comparaison** | Ce binaire mis à côté d'un autre, ce qui est la question qu'on se pose quand un correctif sort et à laquelle aucune autre vue ne répond : une recompilation déplace toute l'image, chaque adresse est neuve, et une comparaison d'octets annonce que rien n'a survécu. Les fonctions sont d'abord appariées — par un nom que les deux fichiers portent, par la même adresse avec les mêmes octets, par les mêmes octets où qu'ils soient, par la même forme une fois écartés les nombres que portent les instructions, ou en étant la seule fonction non appariée qu'appelle chaque côté d'une paire déjà établie —, et chaque ligne dit lequel de ces chemins l'a produite, parce qu'un nom et un corps sont des faits quand une forme et un voisinage sont des lectures. Puis ce qui a changé : l'écart entre deux corps, compté sur un alignement qui ignore ces nombres, pour qu'une compilation à une autre adresse de base ne soit pas annoncée comme un programme réécrit ; les fonctions qu'un fichier a et que l'autre n'a pas ; et les chaînes, les bibliothèques liées et les sections que l'un des deux détient seul. Un appariement ambigu est refusé plutôt que choisi : trois talons identiques de chaque côté s'apparient de six façons, et en retenir une serait une conclusion que rien ne soutient. |
...
**Décompilateur.** Celui de Desdec, en six passes sur l'analyse qu'il fait
déjà — l'élévation, la convention d'appel, la substitution et la suppression
des écritures mortes, les dominateurs et les arcs retour, puis le C. Il ne
dépend que du fichier ouvert, répond en une milliseconde environ par
fonction, et n'exécute aucun octet. Et Rizin avec rz-ghidra, ou RetDec, quand
l'un d'eux est installé et choisi — le moteur qui a produit le texte est
toujours nommé, et le désassemblage correspondant est à un bouton de là.
...
épingler, sous `~/.local/share`. Sous macOS, il écrit un bundle d'application
dans `~/Applications`, ce qui met Desdec dans le Dock, dans Spotlight et dans
le Launchpad. `--no-desktop` n'installe que le binaire.
...
Sous Windows, il écrit un raccourci du menu Démarrer portant la même icône,
demandée au binaire en `.ico` ; `-NoShortcut` n'installe que le binaire.
...
Pour installer ce que le dépôt construit — le binaire sur votre `PATH`, son
icône et une entrée de menu qu'un dock peut épingler — et pour le retirer :

```sh
scripts/insl.sh              # ~/.local/bin/desdec, icône, entrée de menu, ligne PATH
scripts/insl.sh --build      # recompile d'abord, même si target/release en a un
scripts/unsl.sh --dry-run    # dit ce qui partirait, ne retire rien
scripts/unsl.sh              # le binaire, l'icône, l'entrée de menu et cette ligne PATH
scripts/unsl.sh --purge      # et les notes, préférences et le catalogue de bibliothèques
```

Les deux acceptent `--prefix` et `--name`, de sorte qu'une compilation de
développement peut cohabiter avec une release installée. `insl.sh` vérifie que
ce qu'il installe est bien un ELF 64 bits pour cette machine, et `unsl.sh`
refuse de retirer ce qui n'est pas Desdec — `--name ls --prefix /usr/bin` ne
retire rien. Vos notes et préférences survivent à une désinstallation
ordinaire ; `--purge` est le drapeau qui dit le contraire, et il nomme chaque
répertoire qu'il vide.
...
Un troisième fichier vous appartient de la même façon : les descriptions que
vous écrivez des bibliothèques que vous rencontrez, dans
`desdec/libraries.conf`, une par ligne sous la forme `nom = explication`. Elles
l'emportent sur le catalogue intégré, et le fichier se modifie depuis Desdec —
Préférences → Comportement l'ouvre dans une zone de texte, et le bouton « ? »
d'une bibliothèque non décrite ouvre la même zone avec son nom déjà inscrit.
L'enregistrement le relit, si bien que la pression suivante dit la nouvelle
phrase.
