...
translations! {
    ...
    ReferencesHelp => [
        "Les instructions sont exactes ; un mot de données qui contient cette adresse est un pointeur probable — table de fonctions, relocation — mais ce peut aussi être un nombre qui y ressemble.",
        "The instructions are exact; a data word holding this address is a likely pointer — a function table, a relocation — but it may also be a number that looks like one.",
        "Las instrucciones son exactas; una palabra de datos que contiene esta dirección es un puntero probable — tabla de funciones, reubicación — pero también puede ser un número que lo parece.",
    ],
