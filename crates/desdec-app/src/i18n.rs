...
translations! {
    GuidedAnalysis => ["analyse guidée", "guided analysis", "análisis guiado"],
    ExpertAnalysis => ["analyse experte", "expert analysis", "análisis experto"],
    Menu => ["Menu", "Menu", "Menú"],
    ...
    Guided => ["Guidé", "Guided", "Guiado"],
    Expert => ["Expert", "Expert", "Experto"],
    ...
    NextStep => ["Étape suivante", "Next step", "Siguiente paso"],
    NextStepDetail => [
    ...
    ToggleExpertMode => [
        "Basculer le mode guidé / expert",
        "Toggle guided / expert mode",
        "Alternar modo guiado / experto",
    ],
    ...
    DenseCodeWarning => [
        "Une section exécutable est très dense. Le code est probablement compressé ou chiffré, et le désassemblage direct donnera peu de résultats.",
        "An executable section is very dense. The code is likely compressed or encrypted, and disassembling it directly will reveal little.",
        "Una sección ejecutable es muy densa. El código probablemente esté comprimido o cifrado, y desensamblarlo directamente revelará poco.",
    ],
    DenseCodeHint => [
    ...
    SegmentsHelp => [
        "Chaque section est une région du fichier chargée en mémoire avec ses propres droits. Le code exécutable porte le droit x.",
        "Each section is a region of the file loaded into memory with its own rights. Executable code carries the x right.",
        "Cada sección es una región del archivo cargada en memoria con sus propios permisos. El código ejecutable lleva el permiso x.",
    ],
    StringsHelp => [
        "Les chaînes lisibles révèlent souvent chemins, messages et URL utilisés par le programme.",
    ...
    ExpertHint => [
        "Passez en mode expert pour afficher le mappage mémoire, les protections et les dépendances.",
        "Switch to expert mode to show the load mapping, hardening and dependencies.",
        "Cambie al modo experto para mostrar el mapeo de memoria, las protecciones y las dependencias.",
    ],
