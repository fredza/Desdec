...
translations! {
    ...
    NasmSyntax => ["Syntaxe NASM", "NASM syntax", "Sintaxis NASM"],
    NasmSyntaxHelp => [
        "Le listing s’écrit à la manière d’Intel (la destination d’abord, « mov rax, [rbp-8] ») au lieu de celle d’AT&T. Chaque ligne est redécodée depuis ses propres octets, jamais traduite depuis le texte AT&T. Sans effet sur ARM64.",
        "The listing is written Intel’s way (destination first, “mov rax, [rbp-8]”) instead of AT&T’s. Each line is decoded again from its own bytes, never translated from the AT&T text. No effect on ARM64.",
        "El listado se escribe al modo de Intel (primero el destino, «mov rax, [rbp-8]») en lugar del de AT&T. Cada línea se descodifica de nuevo a partir de sus propios bytes, nunca se traduce del texto AT&T. Sin efecto en ARM64.",
    ],
    Persistence => [
    ...
    // The programmer's calculator: one value, read in every base at once, and
    // the bit operations a reader does by hand otherwise.
    Calculator => ["Calculatrice", "Calculator", "Calculadora"],
    CalculatorHelp => [
        "Une seule valeur, lue dans toutes les bases à la fois : modifiez n’importe quel champ, les autres suivent. Elle se calcule comme sur une calculatrice — un nombre, une opération, un nombre, « = ». Un octet non imprimable s’écrit « · » dans la chaîne.",
        "One value, read in every base at once: change any field and the others follow. It is worked out the way a calculator is used — a number, an operation, a number, “=”. A byte that cannot be printed is written “·” in the string.",
        "Un solo valor, leído en todas las bases a la vez: modifique cualquier campo y los demás lo siguen. Se calcula como en una calculadora: un número, una operación, un número, «=». Un byte no imprimible se escribe «·» en la cadena.",
    ],
    BitWidth => ["Largeur", "Width", "Anchura"],
    Hexadecimal => ["Hexadécimal", "Hexadecimal", "Hexadecimal"],
    Decimal => ["Décimal", "Decimal", "Decimal"],
    Octal => ["Octal", "Octal", "Octal"],
    AsciiText => ["Chaîne ASCII", "ASCII string", "Cadena ASCII"],
    TooWideForTheWidth => [
        "Trop grand pour la largeur choisie : rien n’a été changé.",
        "Too large for the chosen width: nothing was changed.",
        "Demasiado grande para la anchura elegida: no se ha cambiado nada.",
    ],
    NotReadableHere => [
        "Illisible dans cette base : rien n’a été changé.",
        "Cannot be read in this base: nothing was changed.",
        "Ilegible en esta base: no se ha cambiado nada.",
    ],
    Bits => ["Bits", "Bits", "Bits"],
    SetBits => ["Bits à 1", "Set bits", "Bits a 1"],
    Shifts => ["Décalages", "Shifts", "Desplazamientos"],
    ShiftLeft => ["Décalage à gauche", "Shift left", "Desplazamiento a la izquierda"],
    ShiftRight => [
        "Décalage à droite (logique : entre des zéros)",
        "Shift right (logical: shifts in zeroes)",
        "Desplazamiento a la derecha (lógico: entran ceros)",
    ],
    ShiftRightSigned => [
        "Décalage à droite (arithmétique : garde le signe)",
        "Shift right (arithmetic: keeps the sign)",
        "Desplazamiento a la derecha (aritmético: conserva el signo)",
    ],
    RotateLeft => ["Rotation à gauche", "Rotate left", "Rotación a la izquierda"],
    RotateRight => ["Rotation à droite", "Rotate right", "Rotación a la derecha"],
    Add => ["Addition", "Add", "Suma"],
    Subtract => ["Soustraction", "Subtract", "Resta"],
    Multiply => ["Multiplication", "Multiply", "Multiplicación"],
    Divide => [
    ...
    Remainder => [
        ...
        "Remainder of the integer division, unsigned",
        "Resto de la división entera, sin signo",
    ],
    NoDivisionByZero => [
        "Une division par zéro n’a pas de réponse.",
        "A division by zero has no answer.",
        "Una división por cero no tiene respuesta.",
    ],
    ArithmeticWraps => [
        "L’arithmétique reste dans la largeur : ce qui déborde est perdu, comme sur le processeur.",
        "The arithmetic stays inside the width: what overflows is lost, as it is on the processor.",
        "La aritmética permanece dentro de la anchura: lo que desborda se pierde, como en el procesador.",
    ],
    EntryBase => ["Saisie", "Entry", "Entrada"],
    WorkItOut => ["Calculer", "Work it out", "Calcular"],
    ChooseAnOperationFirst => [
        "Choisissez d’abord une opération.",
        "Choose an operation first.",
        "Elija primero una operación.",
    ],
    Backspace => [
    ...
    NotADigitInThisBase => [
        "Ce chiffre n’existe pas dans cette base.",
        "That digit does not exist in this base.",
        "Ese dígito no existe en esta base.",
    ],
    BitwiseAnd => ["ET bit à bit", "Bitwise AND", "Y bit a bit"],
    BitwiseOr => ["OU bit à bit", "Bitwise OR", "O bit a bit"],
    BitwiseXor => ["OU exclusif bit à bit", "Bitwise XOR", "O exclusivo bit a bit"],
    BitwiseOperations => ["Opérations bit à bit", "Bitwise operations", "Operaciones bit a bit"],
    BitwiseNot => ["Complément : tous les bits inversés", "Complement: every bit inverted", "Complemento: todos los bits invertidos"],
    Negate => ["Opposé, en complément à deux", "Negate, in two's complement", "Opuesto, en complemento a dos"],
    SwappedBytes => ["Octets inversés", "Byte-swapped", "Bytes invertidos"],
    SwapBytes => [
        "Inverser l’ordre des octets (gros-boutiste ↔ petit-boutiste)",
        "Reverse the byte order (big-endian ↔ little-endian)",
        "Invertir el orden de los bytes (big-endian ↔ little-endian)",
    ],
    ClearValue => ["Remettre à zéro", "Clear to zero", "Poner a cero"],
    ...
    Compare => ["Comparaison", "Compare", "Comparación"],
    CompareIntro => [
        "Deux binaires côte à côte : ce qui n’a pas bougé, ce qui a été déplacé, ce qui a changé. Le fichier ouvert est celui de gauche ; choisissez celui à mettre en face.",
        "Two binaries side by side: what has not moved, what was moved, what changed. The open file is the one on the left; choose the one to set against it.",
        "Dos binarios uno al lado del otro: lo que no se ha movido, lo que se ha desplazado, lo que ha cambiado. El archivo abierto es el de la izquierda; elija el que quiere poner enfrente.",
    ],
    CompareChooseOther => ["Choisir l’autre binaire", "Choose the other binary", "Elegir el otro binario"],
    CompareReplaceOther => ["Choisir un autre fichier", "Choose another file", "Elegir otro archivo"],
    CompareForget => ["Fermer la comparaison", "Close the comparison", "Cerrar la comparación"],
    CompareReading => ["Lecture de l’autre binaire…", "Reading the other binary…", "Leyendo el otro binario…"],
    CompareSameFile => [
        "Les deux fichiers ont la même empreinte : ce sont les mêmes octets.",
        "Both files have the same digest: they are the same bytes.",
        "Ambos archivos tienen la misma huella: son los mismos bytes.",
    ],
    CompareDigestUnknown => [
        "L’un des deux fichiers est trop grand pour être lu en entier, donc il n’a pas d’empreinte : la comparaison porte sur la partie lue.",
        "One of the two files is too large to be read whole and so has no digest: the comparison is of the part that was read.",
        "Uno de los dos archivos es demasiado grande para leerse entero y no tiene huella: la comparación es de la parte leída.",
    ],
    CompareNoDifference => [
        "Rien ne distingue les deux fichiers dans ce que Desdec en lit.",
        "Nothing tells the two files apart in what Desdec reads of them.",
        "Nada distingue los dos archivos en lo que Desdec lee de ellos.",
    ],
    CompareMine => ["Le fichier ouvert", "The open file", "El archivo abierto"],
    CompareTheirs => ["L’autre fichier", "The other file", "El otro archivo"],
    CompareIdentical => ["Identiques", "Identical", "Idénticas"],
    CompareChanged => ["Modifiées", "Changed", "Modificadas"],
    CompareOnlyMine => ["Retirées", "Removed", "Eliminadas"],
    CompareOnlyTheirs => ["Ajoutées", "Added", "Añadidas"],
    CompareMoved => ["déplacée", "moved", "desplazada"],
    CompareMovedHint => [
        "La fonction est à une autre adresse dans l’autre fichier.",
        "The function sits at another address in the other file.",
        "La función está en otra dirección en el otro archivo.",
    ],
    CompareHideIdentical => [
        "Masquer les fonctions inchangées",
        "Hide the unchanged functions",
        "Ocultar las funciones sin cambios",
    ],
    CompareHiddenCount => [
        "fonctions inchangées masquées",
        "unchanged functions hidden",
        "funciones sin cambios ocultas",
    ],
    ComparePairing => ["Appariement", "Pairing", "Emparejamiento"],
    ComparePairingHint => [
        "Comment les deux fonctions ont été reconnues comme la même. Un nom que les deux fichiers portent et des octets identiques sont des faits ; une forme et un voisinage sont des lectures.",
        "How the two functions were taken to be the same one. A name both files carry and identical bytes are facts; a shape and a neighbour are readings.",
        "Cómo se han reconocido las dos funciones como la misma. Un nombre que ambos archivos llevan y unos bytes idénticos son hechos; una forma y una vecindad son lecturas.",
    ],
    ComparePairingName => ["par le nom", "by name", "por el nombre"],
    ComparePairingAddress => ["même endroit, mêmes octets", "same place, same bytes", "mismo lugar, mismos bytes"],
    ComparePairingBytes => ["par les octets", "by its bytes", "por los bytes"],
    ComparePairingShape => ["par la forme", "by its shape", "por la forma"],
    ComparePairingNeighbour => ["par un voisin", "by a neighbour", "por un vecino"],
    CompareDistance => ["Écart", "Distance", "Distancia"],
    CompareDistanceHint => [
        "Les instructions qu’un côté a et que l’autre n’a pas. Les nombres que porte une instruction ne comptent pas : une recompilation les déplace tous, et un écart nul sur une fonction modifiée dit exactement cela — mêmes instructions, autres valeurs.",
        "The instructions one side holds and the other does not. The numbers an instruction carries are not counted: a recompilation moves all of them, and a distance of nothing on a changed function says exactly that — the same instructions, other values.",
        "Las instrucciones que un lado tiene y el otro no. Los números que lleva una instrucción no cuentan: una recompilación los desplaza todos, y una distancia nula en una función modificada dice exactamente eso: las mismas instrucciones, otros valores.",
    ],
    CompareDistanceUnmeasured => ["non mesuré", "not measured", "no medido"],
    CompareDistanceUnmeasuredHint => [
        "Les deux corps sont trop longs pour être alignés. Ce n’est pas un écart nul : la question n’a pas été posée.",
        "The two bodies are too long to be aligned. This is not a distance of nothing: the question was not put.",
        "Los dos cuerpos son demasiado largos para alinearse. No es una distancia nula: la pregunta no se planteó.",
    ],
    CompareOnlyLibraries => ["Bibliothèques liées", "Linked libraries", "Bibliotecas enlazadas"],
    CompareOnlyStrings => ["Chaînes", "Strings", "Cadenas"],
    CompareSectionsTitle => ["Sections", "Sections", "Secciones"],
    CompareAbsent => ["absente", "absent", "ausente"],
    CompareTruncatedList => [
        "La liste s’arrête à la limite : les deux fichiers diffèrent de plus que ce qui est montré.",
        "The list stops at the bound: the two files differ by more than what is shown.",
        "La lista se detiene en el límite: los dos archivos difieren en más de lo que se muestra.",
    ],
    CompareOpenFileChanged => [
        "Le fichier ouvert a changé sur le disque depuis son analyse : la comparaison porterait sur des octets qui ne sont plus ceux affichés. Rouvrez-le, puis recommencez.",
        "The open file has changed on disk since it was analysed: the comparison would be of bytes that are no longer the ones on screen. Reopen it, then start again.",
        "El archivo abierto ha cambiado en el disco desde que se analizó: la comparación sería de bytes que ya no son los que están en pantalla. Vuelva a abrirlo y empiece de nuevo.",
    ],
    CompareNoOtherFile => [
        "Aucun autre fichier n’est ouvert en face.",
        "No other file is set against this one.",
        "No hay ningún otro archivo enfrente.",
    ],
    CompareFilterHint => ["Filtrer par nom", "Filter by name", "Filtrar por nombre"],
    CompareGoTo => [
        "Ouvre cette fonction dans le listing du fichier ouvert.",
        "Opens this function in the open file’s listing.",
        "Abre esta función en el listado del archivo abierto.",
    ],
    CompareNothingToShow => [
        "Aucune ligne ne répond à ce filtre.",
        "No row answers this filter.",
        "Ninguna fila responde a este filtro.",
    ],
